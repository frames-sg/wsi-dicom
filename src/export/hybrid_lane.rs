...
pub(crate) fn export_dicom_instance_jobs_prefer_device_htj2k_hybrid_lanes(
    ...
    let mut reports = std::thread::scope(|scope| -> Result<Vec<_>, WsiDicomError> {
        ...
        let writer_handle = scope.spawn(move || {
            ...
            Ok::<_, WsiDicomError>(writer_reports)
        });
        ...
        for job in gpu_jobs {
            ...
            writer_tx
                ...
                .map_err(|_| WsiDicomError::DicomWrite {
        ...
        let mut lane_reports = writer_handle
            .join()
            .map_err(|_| WsiDicomError::DicomWrite {
                path: request.output_dir.clone(),
                message: "DICOM writer lane panicked".into(),
            })??;
        let cpu_reports = cpu_handle.join().map_err(|_| WsiDicomError::Encode {
