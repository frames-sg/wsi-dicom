...
const WORKLOAD_SPECS: [WorkloadSpec; 10] = [
    WorkloadSpec {
        name: "cold_open",
        target_n: 10,
        gate_mode: "gating",
    ...
    WorkloadSpec {
        ...
        target_n: 100,
        gate_mode: "gating",
        comparability: "statumen-only",
        comparability_note: Some(
            "OpenSlide C API has no raw compressed tile API; each Statumen sample performs 32 raw-tile reads to keep fast passthrough paths above timer resolution",
        ),
    },
    WorkloadSpec {
        name: "pan_trace_l0",
    ...
    WorkloadSpec {
        name: "associated_macro",
        target_n: 30,
        gate_mode: "gating",
        comparability: "exact",
        comparability_note: Some(
            "Selected single-workload coverage for slides exposing a macro associated image; records an explicit error when absent",
        ),
    },
];
...
mod tests {
    ...
    #[test]
    fn raw_tile_l0_spec_is_statumen_only_and_gating() {
        let spec = workload_spec("raw_tile_l0").expect("raw_tile_l0 spec");
        assert_eq!(spec.target_n, 100);
        assert_eq!(spec.gate_mode, "gating");
        assert_eq!(spec.comparability, "statumen-only");
        assert!(spec.comparability_note.is_some());
    }
