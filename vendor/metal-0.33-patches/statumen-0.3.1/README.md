...
`statumen` is a Rust whole-slide image (WSI) reader. Its built-in registry
opens TIFF-family vendor slides, DICOM VL WSI files and series, Zeiss CZI/ZVI,
MIRAX, Hamamatsu VMS/VMU, Olympus VSI/ETS, raw JPEG 2000 codestream fixtures,
and `.svcache` containers.

The crate owns container probing, metadata normalization, slide / scene /
series / level geometry, tile addressing, region composition, associated
images, raw compressed-tile access, and `.svcache` read-through policy. JPEG,
JPEG 2000, and HTJ2K codec work is delegated to the `signinum-*` crates.
...
cargo xtask perf-profile path/to/slide.svs single_tile_l0
...
Use `cargo xtask perf-capture` for before/after local regression evidence. It
runs `wsi_bench` three times per slide by default, writes ignored JSON under
`bench/results/local-regression/`, and can use explicit slide paths or
`STATUMEN_PERF_SLIDES` for private WSI/DICOM coverage. Captures include git,
toolchain, host CPU/GPU, feature, RUSTFLAGS, cache/thread env, corpus tier,
workload, repeat, and raw sample metadata. `perf-compare` checks p50, p95,
p99, mean, and peak RSS when available and fails when after/before exceeds
1.05 in at least 2 of 3 comparable runs. For maintainability-only PRs, the
expected result is no flagged regression.

Use `cargo xtask perf-capture-openslide` for first-class competitor evidence
against OpenSlide. It requires real WSI slide paths or `STATUMEN_PERF_SLIDES`
because the default JP2K codestream fixture is not an OpenSlide slide. Compare
OpenSlide as the `before` JSON and Statumen as the `after` JSON; the reported
ratio is then Statumen/OpenSlide, and values below `1.0` mean Statumen is
faster.

Use `cargo xtask perf-profile` to print `samply` and `xcrun xctrace` profiling
recipes. CPU profiling defaults to `samply`; Metal work should use the Metal
System Trace recipe when macOS Metal support is available. Flamegraphs are
optional diagnostics, not benchmark evidence by themselves.
...
cargo xtask coverage-changed --base origin/main --lcov lcov.info --threshold 80
```
...
are set.
CI enforces changed-path coverage for instrumented non-test library and shim
Rust source files and uploads the full LCOV report as an artifact.
