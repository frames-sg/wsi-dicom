...
fn repository_hygiene_files_match_public_release_policy() {
    ...
    let root_architecture =
        fs::read_to_string(crate_root().join("architecture.md")).expect("read root architecture");
    assert!(
        root_architecture.contains("docs/architecture.md")
            && root_architecture.lines().count() <= 30,
        "root architecture.md should stay a short workspace pointer to docs/architecture.md"
    );
}
