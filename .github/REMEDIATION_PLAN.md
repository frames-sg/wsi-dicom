...
| 2026-08-08 | 0.7.2 correctness, calibration, and release baseline | Implemented and verified locally; publication frozen | Formatting, warnings-denied Clippy, strict docs, debug and release workspace matrices (346 passed and 7 protected fixtures ignored in each), 83.84% line coverage, 37 Python policy tests, exact semver allowlist, RustSec/deny/vet, isolated crate tests, credential-free publish dry-run, strict doctor/self-test, and required JPEG/JPEG 2000/HTJ2K plus DICOM validator tests passed. No commit, tag, hosted workflow rehearsal, clean-clone run, real-slide/PACS/viewer, GPU-hardware, or clinical workflow validation was performed. | Obtain independent review and commit authorization; run clean CI/rehearsal and protected clinical validation before any tag or deployment |
