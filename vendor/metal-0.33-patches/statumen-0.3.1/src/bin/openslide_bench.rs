...
fn main() {
    ...
    let assoc_names = slide.associated_names();
    let has_macro = assoc_names.iter().any(|n| n == "macro");
    if selected_workload.as_deref() == Some("associated_macro") {
        if has_macro {
            workloads.push(run_workload::<_, String>("associated_macro", 30, || {
                slide.read_associated("macro")
            }));
        } else {
    ...
    if should_run_workload(selected_workload.as_deref(), "raw_tile_l0") {
        workloads.push(WorkloadResult::with_error(
            "raw_tile_l0",
            "OpenSlide C API does not expose raw compressed tile access",
        ));
    }
