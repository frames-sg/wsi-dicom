...
fn tile_requests(slide: &Slide, max_tiles: usize) -> Result<Vec<TileRequest>, String> {
    if let (Ok(level), Ok(col), Ok(row)) = (
        env::var("STATUMEN_DICOM_BENCH_LEVEL"),
        env::var("STATUMEN_DICOM_BENCH_COL"),
        env::var("STATUMEN_DICOM_BENCH_ROW"),
    ) {
        let level = level
            .parse::<u32>()
            .map_err(|err| format!("invalid STATUMEN_DICOM_BENCH_LEVEL: {err}"))?;
        let col = col
            .parse::<i64>()
            .map_err(|err| format!("invalid STATUMEN_DICOM_BENCH_COL: {err}"))?;
        let row = row
            .parse::<i64>()
            .map_err(|err| format!("invalid STATUMEN_DICOM_BENCH_ROW: {err}"))?;
        return Ok((0..max_tiles)
            .map(|offset| TileRequest {
                scene: 0,
                series: 0,
                level,
                plane: PlaneSelection::default(),
                col: col + offset as i64,
                row,
            })
            .collect());
    }
