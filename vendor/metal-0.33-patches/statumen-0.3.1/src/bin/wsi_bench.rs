...
use statumen::{
    LevelIdx, PlaneIdx, PlaneSelection, RegionRequest, SceneId, SeriesId, Slide, TileRequest,
    TileViewRequest,
};
use std::path::PathBuf;

const RAW_TILE_INNER_READS: usize = 32;
...
fn main() {
    ...
    let has_macro = handle.dataset().associated_images.contains_key("macro");
    if selected_workload.as_deref() == Some("associated_macro") {
        if has_macro {
            workloads.push(run_workload::<_, statumen::WsiError>(
                "associated_macro",
                30,
                || handle.read_associated("macro"),
            ));
        } else {
            workloads.push(WorkloadResult::with_error(
                "associated_macro",
                "slide does not expose a macro associated image",
            ));
        }
    }

    if should_run_workload(selected_workload.as_deref(), "raw_tile_l0") {
        match raw_tile_request_l0(&handle, plan.center_l0) {
            Ok(req) => workloads.push(run_workload::<_, statumen::WsiError>(
                "raw_tile_l0",
                100,
                || {
                    for _ in 0..RAW_TILE_INNER_READS {
                        let raw = handle.read_raw_compressed_tile(&req)?;
                        std::hint::black_box(raw.data.len());
                    }
                    Ok(())
                },
            )),
            Err(err) => workloads.push(WorkloadResult::with_error("raw_tile_l0", err)),
        }
    }
...
fn raw_tile_request_l0(handle: &Slide, center_l0: (i64, i64)) -> Result<TileRequest, String> {
    let level = &handle.dataset().scenes[0].series[0].levels[0];
    let hit = level
        .tile_layout
        .tiles_for_region(center_l0.0, center_l0.1, 1, 1)
        .into_iter()
        .next()
        .ok_or_else(|| {
            format!(
                "level 0 raw tile lookup found no tile covering center pixel ({}, {})",
                center_l0.0, center_l0.1
            )
        })?;
    Ok(TileRequest {
        scene: 0,
